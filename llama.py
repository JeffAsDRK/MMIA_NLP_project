...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.attention(h) + x  # Steps 1-3: normalize, attention, residual
        attn_out = h  # Save unnormalized attention output for step 6
        h = self.ffn_norm(h)  # Step 4: normalize attention output
        h = self.feed_forward(h) + attn_out  # Steps 5-6: feedforward + residual from unnormalized attention output
        return h
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, epsilon=0.05):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                '''
                probs = F.softmax(logits / temperature, dim=-1)  # Step 1
                mask = probs >= epsilon  # Step 2
                filtered_probs = probs * mask  # Step 3
                filtered_probs = filtered_probs / filtered_probs.sum(dim=-1, keepdim=True)
                idx_next = torch.multinomial(filtered_probs, num_samples=1)  # Step 5
